...
const MainLayout = () => {
    ...
    const apiUrl = (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') ? 'http://localhost:3000' : 'https://api.aegism.online';
    ...
    const fetchUserProfile = async () => {
        try {
            ...
            } else if (res.status === 401) { localStorage.clear(); navigate('/login'); }
    ...
    const updateUserUI = (u: any) => {
        ...
        if (isSuper) { roleDisplay = 'Super Admin'; permissions = ['ALL']; }
            ...
            try { permissions = Array.isArray(u.role.permissions) ? u.role.permissions : JSON.parse(u.role.permissions); } catch { permissions = []; }
        }
    ...
    const handleLogout = () => {
        Swal.fire({ title: 'Đăng xuất?', text: 'Bạn có chắc muốn thoát?', icon: 'question', showCancelButton: true, confirmButtonColor: '#2563EB', confirmButtonText: 'Đăng xuất', cancelButtonText: 'Ở lại' })
            .then((result) => { if (result.isConfirmed) { localStorage.clear(); navigate('/login'); } });
    };
            ...
            <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-gray-200 transition-transform duration-300 ease-in-out flex flex-col justify-between shadow-lg lg:static lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                <div>
                    <div className="flex items-center justify-between h-20 border-b border-gray-100 px-4">
                        <Link to="/">
                            <img src="/img/aegism_logo_mini.png" alt="Logo" className="h-[30px] w-auto" onError={(e) => e.currentTarget.style.display = 'none'} />
                        </Link>
                        <button onClick={() => setSidebarOpen(false)} className="lg:hidden p-2 rounded-full hover:bg-red-50 hover:text-red-500 text-gray-400 transition-colors">
                            <HiOutlineX className="w-5 h-5" />
                        </button>
                    </div>
                    <nav className="mt-6 px-4 space-y-1 overflow-y-auto max-h-[calc(100vh-160px)]">
                        <Link to="/dashboard" onClick={() => setSidebarOpen(false)} className={getLinkClass('/')}><HiOutlineHome className="w-5 h-5 mr-3" />Tổng quan (Dashboard)</Link>
                ...
                <div className="p-4 border-t border-gray-200">
                    <button onClick={handleLogout} className="flex items-center w-full px-4 py-2 text-sm text-gray-600 hover:text-red-600 transition-colors rounded-lg hover:bg-red-50">
                        <HiOutlineLogout className="w-5 h-5 mr-3" />Đăng xuất
                    </button>
                </div>
            </aside>
                ...
                <header className="flex justify-between items-center py-4 px-6 bg-white border-b border-gray-200 shadow-sm relative z-40 h-20">
                    ...
                    <div className="flex items-center space-x-6">
                        ...
                        <div className="relative group h-full flex items-center">
                            ...
                            <div className="absolute right-0 top-full mt-1 w-80 bg-white rounded-xl shadow-2xl border border-gray-100 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                                <div className="p-6">
                                    <div className="flex items-center space-x-4 mb-4">
                                        <img className="h-14 w-14 rounded-full object-cover border-2 border-blue-500 p-0.5" src={user.avatar} alt="Avatar" />
                                        <div>
                                    ...
                                    <div className="space-y-2 text-sm text-gray-600">
                                        <div>{user.email}</div>
                                        <div>Tenant: <span className="font-semibold">{user.tenantName}</span></div>
                                    </div>
                                    <div className="mt-4 pt-3 border-t border-gray-100">
                                        <Link to="/profile" className="text-xs text-blue-600 hover:text-blue-800 font-medium">Cài đặt tài khoản</Link>
                                    </div>
                                </div>
                            </div>
                        </div>
