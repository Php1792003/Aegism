    ...
    return (
        <div className="flex h-screen bg-gray-50 overflow-hidden font-sans text-gray-800">
            ...
            <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-gray-200 transition-transform duration-300 ease-in-out flex flex-col justify-between shadow-lg lg:static lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                <div>
                    {/* Header sidebar với nút X */}
                    <div className="relative flex items-center justify-center h-20 border-b border-gray-100 px-4">
                        <Link to="/" className="flex items-center justify-center">
                            <img src="/img/aegism_logo_mini.png" alt="Logo" className="h-[30px] w-auto object-contain"
                                onError={(e) => e.currentTarget.style.display = 'none'} />
                        </Link>
                        {/* Nút X absolute để không ảnh hưởng căn giữa logo */}
                        <button
                            onClick={() => setSidebarOpen(false)}
                            className="lg:hidden absolute right-3 top-1/2 -translate-y-1/2 p-2 rounded-full hover:bg-red-50 hover:text-red-500 text-gray-400 transition-colors"
                        >
                    ...
                    <nav className="mt-6 px-4 space-y-1 overflow-y-auto max-h-[calc(100vh-160px)] custom-scrollbar">
                        ...
                        <Link to="/qrcodes" onClick={() => setSidebarOpen(false)} className={getLinkClass('/qrcodes')}>
                            <HiOutlineQrcode className="w-5 h-5 mr-3" />
                            Mã QR & Điểm quét
                        </Link>
                        <Link to="/staff" onClick={() => setSidebarOpen(false)} className={getLinkClass('/staff')}>
                            <HiOutlineUsers className="w-5 h-5 mr-3" />
                            Nhân sự & Phân quyền
                        </Link>
                        <Link to="/tasks" onClick={() => setSidebarOpen(false)} className={getLinkClass('/tasks')}>
                            <HiOutlineClipboardList className="w-5 h-5 mr-3" />
                            Công việc & Task
                        </Link>
                        <Link to="/chat" onClick={() => setSidebarOpen(false)} className={getLinkClass('/chat')}>
                        ...
                        {['professional', 'enterprise'].includes(currentPlan) && (
                            <div className="pt-2 mt-2 border-t border-gray-100">
                                ...
                                <Link to="/reports" onClick={() => setSidebarOpen(false)} className={getLinkClass('/reports', 'purple')}>
                                    <HiOutlineChartBar className="w-5 h-5 mr-3" />
                                    Báo cáo Thống kê
                                </Link>
                                <Link to="/audit-log" onClick={() => setSidebarOpen(false)} className={getLinkClass('/audit-log', 'purple')}>
                        ...
                        {currentPlan === 'enterprise' && (
                            <div className="pt-2 mt-2 border-t border-gray-100">
                                ...
                                <Link to="/api-integration" onClick={() => setSidebarOpen(false)} className={getLinkClass('/api-integration')}>
                                    <HiOutlineCubeTransparent className="w-5 h-5 mr-3" />
                                    Tích hợp API
                                </Link>
                                <Link to="/branding" onClick={() => setSidebarOpen(false)} className={getLinkClass('/branding')}>
