...
const getNotifLink = (n: any): string => {
    ...
    if (title.includes('chat') || title.includes('tin nhắn')) return '/chat';
    if (t === 'WARNING' || t === 'ERROR') return '/tasks';
...
const MainLayout = () => {
    ...
    useEffect(() => {
        ...
        if (!token) return;
        fetchNotifications();
        const es = new EventSource(`${apiUrl}/api/notifications/stream?token=${token}`);
        es.onmessage = (e) => {
            try {
    ...
    const handleLogout = () => {
        Swal.fire({ title: 'Đăng xuất?', text: 'Bạn có chắc muốn thoát phiên làm việc?', icon: 'question', showCancelButton: true, confirmButtonColor: '#2563EB', confirmButtonText: 'Đăng xuất', cancelButtonText: 'Ở lại' })
    ...
    const hasPermission = (perm: string) => {
        if (user.isSuperAdmin || user.isTenantAdmin || user.permissions.includes('ALL')) return true;
            ...
            <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-gray-200 transition-transform duration-300 ease-in-out flex flex-col justify-between shadow-lg lg:static lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                <div>
                    ...
                    <nav className="mt-6 px-4 space-y-1 overflow-y-auto max-h-[calc(100vh-160px)] custom-scrollbar">
                        <Link to="/dashboard" onClick={() => setSidebarOpen(false)} className={getLinkClass('/')}><HiOutlineHome className="w-5 h-5 mr-3" />Tổng quan</Link>
