...
const ProfilePage = () => {
    ...
    const [user, setUser] = useState<any>({
        ...
        role: null, tenant: null, isTenantAdmin: false, isSuperAdmin: false,
    });
    ...
    const fetchProfile = async () => {
        ...
        } catch (e) { console.error('Lỗi fetch profile:', e); }
    };
    ...
    // ✅ FIX: field name 'avatar' + endpoint POST /api/users/avatar
    const handleAvatarChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        const file = e.target.files?.[0];
        if (!file) return;
        if (!file.type.match(/image\/(jpg|jpeg|png|gif|webp)/)) {
            setErrorMsg('Chỉ chấp nhận file ảnh (JPG, PNG, GIF, WEBP)');
            return;
        }
        ...
        try {
            ...
            fd.append('avatar', file); // ✅ field name phải là 'avatar'
            const res = await fetch(`${API_URL}/api/users/avatar`, { // ✅ endpoint đúng
                method: 'POST',
                headers: { 'Authorization': `Bearer ${token}` },
                body: fd,
            });
            if (res.ok) {
                const data = await res.json();
                const avatarUrl = data.avatarUrl?.startsWith('/uploads')
                    ? `${API_URL}${data.avatarUrl}` : data.avatarUrl;
    ...
    const handleSaveProfile = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (passwordData.newPassword) {
                if (passwordData.newPassword !== passwordData.confirmPassword) {
                    setErrorMsg('Mật khẩu xác nhận không khớp.'); setIsLoading(false); return;
                }
                if (passwordData.newPassword.length < 6) {
                    setErrorMsg('Mật khẩu tối thiểu 6 ký tự.'); setIsLoading(false); return;
                }
            }
            const body: any = { fullName: formData.fullName };
            if (passwordData.newPassword) body.password = passwordData.newPassword;
        ...
        } catch { setErrorMsg('Không thể kết nối đến máy chủ.'); }
    ...
    const getAvatarUrl = () => {
        if (!user.avatar) return `https://ui-avatars.com/api/?name=${encodeURIComponent(user.fullName || 'User')}&background=1B6FFF&color=fff&size=128`;
                    ...
                    <div className="absolute -bottom-12 left-8">
                        <div className="relative">
                            <img src={getAvatarUrl()} alt="Avatar"
                                ...
                                onError={(e) => { (e.target as HTMLImageElement).src = `https://ui-avatars.com/api/?name=${encodeURIComponent(user.fullName || 'User')}&background=1B6FFF&color=fff&size=128`; }}
                            />
                            <label className={`absolute bottom-0 right-0 p-1.5 bg-blue-600 rounded-full border-2 border-white hover:bg-blue-700 transition-colors cursor-pointer ${isUploadingAvatar ? 'opacity-70 cursor-not-allowed' : ''}`}>
                ...
                <div className="pt-16 px-8 pb-8">
                    <div className="mb-8">
                        ...
                        <div className="flex items-center gap-2 mt-1 text-blue-600">
                            ...
                            {user.tenant && <span className="text-xs text-gray-400 normal-case font-normal">· {user.tenant.name}</span>}
                        </div>
                    </div>
                    {successMsg && <div className="mb-6 p-3 bg-green-50 border border-green-200 text-green-700 rounded-lg text-sm">✅ {successMsg}</div>}
                    {errorMsg && <div className="mb-6 p-3 bg-red-50 border border-red-200 text-red-700 rounded-lg text-sm">❌ {errorMsg}</div>}
                    <form onSubmit={handleSaveProfile} className="space-y-6">
                        ...
                        <div className="pt-2 flex justify-end gap-3">
                            <button type="button"
                                onClick={() => { setFormData({ fullName: user.fullName, avatar: user.avatar }); setPasswordData({ newPassword: '', confirmPassword: '' }); setSuccessMsg(''); setErrorMsg(''); }}
