        ...
        <div className="flex-1 flex flex-col overflow-hidden bg-gray-50 p-6">
            ...
            <div className="max-w-7xl mx-auto w-full space-y-6">
                <div className="flex justify-between items-center">
                    <h2 className="text-2xl font-bold text-gray-900">Quản lý API Keys</h2>
                    <div className="flex items-center space-x-4">
                        <div className="relative">
                            <input
                                ...
                                className="w-72 pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 shadow-sm text-sm"
                            />
                        ...
                        <table className="w-full text-left whitespace-nowrap">
                            ...
                            <tbody className="divide-y divide-gray-100 text-sm">
                                ...
                                {!loading && apiKeys.length === 0 && (
                                    <tr><td colSpan={5} className="px-6 py-8 text-center text-gray-400">Không tìm thấy API Key nào.</td></tr>
                                )}
                                {!loading && apiKeys.map(key => (
                                    <tr key={key.id} className="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="px-6 py-4">
                                            <p className="font-semibold text-gray-800">{key.tenant.name}</p>
                                            <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-indigo-100 text-indigo-700 uppercase">{key.tenant.subscriptionPlan}</span>
                                        </td>
                                        <td className="px-6 py-4">
                                            <p className="font-medium text-gray-800">{key.createdBy.fullName}</p>
                                        ...
                                        <td className="px-6 py-4">
                                            {key.isActive ? (
                                                ...
                                                <div>
                                                    <span className="px-2.5 py-1 rounded-full text-xs font-bold bg-red-100 text-red-700">○ Revoked</span>
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            {key.isActive && (
                                                <button onClick={() => revokeKey(key.id)}
                                                    className="text-red-600 hover:text-white border border-red-600 hover:bg-red-600 px-4 py-1.5 rounded-lg transition-colors font-semibold text-xs shadow-sm">
                                                    Thu hồi Key
                                                </button>
                                            )}
                                        </td>
                        ...
                        <div className="px-6 py-4 border-t border-gray-100 flex items-center justify-between bg-gray-50">
                            ...
                            <div className="flex gap-2">
                                <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
                                    className="px-3 py-1 border border-gray-300 rounded text-sm disabled:opacity-50 font-medium hover:bg-gray-100">Trước</button>
                                <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages}
                                    className="px-3 py-1 border border-gray-300 rounded text-sm disabled:opacity-50 font-medium hover:bg-gray-100">Sau</button>
                            </div>
                        </div>
                    )}
                </div>
